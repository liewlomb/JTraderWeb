...
def cal_start_date(endDate):
    month = endDate[5:7]
    if month in ['02','03','04','05','06','07','08','09','10']:
        beginMonth = int(endDate[5:7])-1
        endMonth = int(endDate[5:7])
        startDate = str(endDate[:4])+'-0'+str(int(endDate[5:7])-1)+'-01'
    elif month in ['11','12']:
        beginMonth = int(endDate[5:7])-1
        endMonth = int(endDate[5:7])
        startDate = str(endDate[:4])+'-'+str(int(endDate[5:7])-1)+'-01'
    elif month in ['01']:
        beginMonth = 12
        endMonth = int(endDate[5:7])
        startDate = str(int(endDate[:4])-1)+'-12-01'
    res = {'startDate':startDate,'endDate' :endDate,'beginMonth':beginMonth,'endMonth':endMonth}
    return res 

def buying_recovery(endDate):
    ...
    for i in range(len(stock_list)):
        ...
        percent_begin = ((begin_month['Close'].iloc[-1] - begin_month['Close'].iloc[0])/begin_month['Close'].iloc[0])*100
        
        if len(end_month) != 1:
            percent_end = ((end_month['Close'].iloc[-1] - end_month['Close'].iloc[0])/end_month['Close'].iloc[0])*100
